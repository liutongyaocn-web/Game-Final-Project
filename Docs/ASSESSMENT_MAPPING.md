...
| UI and statistics | JU TPS default UI is now present in `LS_Arena_01` as the base interface; custom HUD extensions will add wave, score, kills, survival timer, FPS, objective text, and pickup prompts. | Foundation integrated; custom statistics HUD planned. |
| Game mechanics | JU TPS provides base controller, camera, aiming, shooting, reload, inventory, health/damage, and interaction foundations; Last Stand code provides wave rules, pickups, objectives, win/lose flow, and restart integration. | Player/camera/UI foundation integrated in `LS_Arena_01`; custom gameplay systems planned. |
...
| Animation | JU TPS provides player locomotion/combat/reload/ragdoll and compatible zombie movement/attack/hit/death animation foundations. | JU TPS default player foundation integrated; enemy animation evidence planned later. |
